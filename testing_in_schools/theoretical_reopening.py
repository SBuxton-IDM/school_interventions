    ...
    for i, sim in enumerate(msims):
        ...
        if len(sim.sims) > 1:  # if running multiple param sets, print all of them
            ...
            for sub_sim in sim.sims:
                ...
                school_results[schools_reopening_scenarios[i]]['num_students'] = sub_sim.school_info[ # DJK Duplicate of above?
                    'num_students']
            ...
            for i, scen in enumerate(schools_reopening_scenarios):
                ...
                for j in range(n_seeds):
                    ...
                    if scen == 'as_normal':
                        ...
                        popfile = popfile_new
                    sim = cv.Sim(pars, popfile=popfile, load_pop=True, label=scen) # or label=analysis_name
                ...
                msim.run(reseed=False, noise=0.0, keep_people=False) # , par_args={'maxload': 0.8}
                msim.reduce()
                msims.append(msim)
                es = []
                ms = []
